...
# TRAINING_PICS_FOLDER = 'training/pics'
...
class IaEngineBase:
   ...
   def __visualize_conv_layer(self, productName: str, layer_name: str):
      model = tensorflow.keras.models.load_model(MODEL_PATH + "/" + productName + ".h5")
      layer_output=model.get_layer(layer_name).output
      
      intermediate_model=tensorflow.keras.models.Model(inputs=model.input,outputs=layer_output)
      image = cv2.imread(TRAINING_PICS_FOLDER + "/" + productName + "/validation/0/pic_01.jpg")
...
class CookingIaEngine(IaEngineBase):
   def train(self, productName: str):
      ...
      #self._IaEngineBase__visualize_conv_layer('asd', layer_name)
      #plt.show()
   ...
   def predict(self, product: str, img: str):
      ...
      loadimg = preprocessing.image.load_img( img, target_size=(255, 255) )
...
class ProductIaEngine(IaEngineBase):
   ...
   def train(self):
      ...
      train_dir, validation_dir = self._IaEngineBase__imageReader('foods')
      ...
      print (model.metrics_names)
      print (model.evaluate(training_set, batch_size=10))
      #self._IaEngineBase__visualize_conv_layer('asd')
      #plt.show()
   ...
   def predict(self, img: str):
      ...
      model = load_model(MODEL_PATH + "/foods.h5", custom_objects={'KerasLayer': hub.KerasLayer(url)})
      npimg = np.array(npimg).astype(float)/255
      npimg = cv2.resize(npimg, (IMG_HEIGHT, IMG_WIDTH))

      predict = model.predict( npimg.reshape(-1, IMG_HEIGHT, IMG_WIDTH, 3) )
      return self.lista_nombreProductos[np.argmax(predict[0], axis=-1)]

   


productsIA = ProductIaEngine(['Milanesas', 'ñoquis']) # Le mando como parametro la lista de los nombres ordenada segun la carpeta de entrenamiento para mostrar la comida resultante.
IA = CookingIaEngine()
