...
from ia.ia_engine import IaEngine, ProductIaEngine
...
def predict(request):
    if request.method == "POST":
        ...
        productIA = ProductIaEngine()
        product = productIA.predict('comidas', '/media/administrador/Matu/PAXAPOS/tina-main-modificado/ia/Milanesas/train/0/pic_03.jpg')
        if (product == 0):
            product = 'Milanesas' #request.POST['product']
        elif (product == 1):
            product = 'ñoquis' #request.POST['product']

        IA = IaEngine()
        scorePredict = IA.predict(product, picture_path)
        
        return render(request, "predict/predict.html", {'scorePredict': scorePredict, 'productPredict': product})
    else:
